...
def get_args_parser():
    ...
    parser.add_argument('--blr', type=float, default=0.0005, metavar='LR',
    ...
    parser.add_argument('--norm_beit', default=True, action='store_true',
                        help='Use norm beit')
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='checkpoints/checkpoint_last.pth.tar', help='finetune from checkpoint')
    ...
    parser.add_argument('--output_dir', default='./outcome',
    ...
    parser.add_argument('--resume', default='./outcome/checkpoint_last.pth',
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_interval == 0:
            ...
            log.info(f"Accuracy of the network on the {len(dataset_val)} val images: {test_stats['acc1']:.2f}%")
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    Attn_dist_rec = [AverageMeter() for _ in range(len(model.blocks))]
...
def evaluate(data_loader, model, device, log):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            output = model(images)
            _, preds = torch.max(output, 1)  # 获取预测类别
    ...
    disp.plot(cmap="Blues", values_format=".2f", text_kw={'fontsize': 10}, ax=ax)
    # 保存归一化混淆矩阵
    plt.savefig("flickr_normalized_confusion_matrix.png", dpi=300, bbox_inches='tight')
    # plt.show()
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
