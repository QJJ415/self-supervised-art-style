...
# from mmseg.models.backbones.mae import MAE as VisionTransformer
...
from mmseg.models.backbones.beit import BEiT, BEiTAttention, BEiTTransformerEncoderLayer
...
class MAE(BEiT):
    ...
    def __init__(self,
        ...
        # delete relative position bias table
        # for layer in self.layers:
        #     layer.attn.relative_position_bias_table = None
        #     layer.attn.relative_position_index = None
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            ...
            assert self.out_indices == [len(self.layers) - 1, ]
            if i in self.out_indices:
                out = x[:, 0]
                # B, C = out.shape
                # out = out.reshape(B, C).permute(0, 3, 1, 2).contiguous()
                outs = out

        return outs


class mmseg_vit_small(nn.Module):
    def __init__(self, num_classes=1000, **kwargs):
        super().__init__()
        embed_dims=384
        self.backbone = MAE(
            patch_size=16, embed_dims=embed_dims, num_layers=12, num_heads=6, mlp_ratio=4, qkv_bias=True, **kwargs)
        self.head = nn.Linear(embed_dims, num_classes)

    def forward(self, *args, **kwargs):
        feat = self.backbone(*args, **kwargs)
        return self.head(feat)


class mmseg_vit_base(nn.Module):
    def __init__(self, num_classes=1000, **kwargs):
        super().__init__()
        embed_dims = 768
        self.backbone = MAE(
            patch_size=16, embed_dims=embed_dims, num_layers=12, num_heads=12, mlp_ratio=4,
...
class mmseg_vit_large(nn.Module):
    def __init__(self, num_classes=1000, **kwargs):
        super().__init__()
        embed_dims = 1024
        self.backbone = MAE(
            patch_size=16, embed_dims=embed_dims, num_layers=24, num_heads=16, mlp_ratio=4, qkv_bias=True, **kwargs)
        self.head = nn.Linear(embed_dims, num_classes)
