...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attn=False, return_qkv=False):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        attn = self.mm(q, k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = self.mm(attn, v).transpose(1, 2).reshape(B, N, C)
        ...
        if (not return_attn) and (not return_qkv):
            return x

        if return_attn:
            assert not return_qkv
            return x, attn
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attn=False, return_attn_feat=False):
        if return_attn:
            fea, attn = self.attn(self.norm1(x), return_attn)
        elif return_attn_feat:
            fea, qkv = self.attn(self.norm1(x), return_qkv=True)
        else:
            fea = self.attn(self.norm1(x), return_attn)
        x = x + self.drop_path(fea)
        x = x + self.drop_path(self.mlp(self.norm2(x)))

        if (not return_attn) and (not return_attn_feat):
            return x

        return_tensors = [x, ]
        if return_attn:
            return_tensors.append(attn)

        if return_attn_feat:
            return_tensors.append(fea)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', add1BlockTo=-1, add1BlockToNumLayers=1, feature_from=-1):
        """
        Args:
        ...
        if add1BlockTo > 0:
            depth += add1BlockToNumLayers
            for i in range(add1BlockToNumLayers):
                dpr.append(drop_path_rate)
        ...
        if add1BlockTo > 0:
            for cnt_layer in range(add1BlockTo, depth-add1BlockToNumLayers):
                self.blocks[cnt_layer] = nn.Identity()

        if feature_from > 0:
            assert add1BlockTo < 0
            for cnt_layer in range(feature_from, depth):
                self.blocks[cnt_layer] = nn.Identity()
    ...
    def forward_features(self, x, return_attn=False, record_feat=False, record_feat_attn=False, mask_ids_keep=None):
        ...
        if mask_ids_keep is not None:
            N, L, D = x.shape  # batch, length, dim
            x_patches = torch.gather(x[:, 1:], dim=1, index=mask_ids_keep.unsqueeze(-1).repeat(1, 1, D))
        ...
        for blk in self.blocks:
            if return_attn:
                ...
                if record_feat_attn:
                    x, fea = blk(x, return_attn_feat=record_feat_attn)
                    if record_feat:
                        features.append(fea.clone().detach())
                else:
                    x = blk(x)
                    if record_feat:
                        features.append(x.clone().detach())
        if record_feat:
            self.recorded_feature = features
        x = self.norm(x)
        if self.dist_token is None:
    ...
    def forward(self, x, return_attn=False, record_feat=False, **kwargs):
        x, attns = self.forward_features(x, return_attn, record_feat, **kwargs)
