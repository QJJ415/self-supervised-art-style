...
class NetWrapper(nn.Module):
    def __init__(self, net, embed_size, args, prediction=False, intermediate=False, last_bn=True):
        super().__init__()
        self.net = net
        self.intermediate = intermediate   # true
        self.predictor = None
        ...
        if intermediate is False:
            ...
            self.projector = nn.ModuleList([])   # 这

            if prediction:
                self.predictor = nn.ModuleList([])   # 这
    ...
    def forward(self, x, return_embedding=False, epoch=None):
        if self.intermediate and return_embedding is False:
            # print("intermediate")  # 这句
            representation = self.net.module.get_intermediate_layers(x, 12)  # 中间层
        else:
            # print("111")
        ...
        if self.intermediate:
            ...
            print(representation.shape)	# torch.Size([192, 768])
