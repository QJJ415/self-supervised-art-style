    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    self.log.info(str(log_msg.format(
...
def get_second_path(path, insert_name="_logs4.7"):
    dir = ""
    root=path
    while dir == "":
...
class NativeScalerWithGradNormCountMoe:
    state_dict_key = "amp_scaler"
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, epoch_name=None):
    output_dir = Path(args.output_dir)
    if epoch_name is None:
        epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('checkpoint_{}.pth'.format(epoch_name))]
...
def save_model_best(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        ...
        client_state = {'epoch': epoch}
        model.save_checkpoint(save_dir=args.output_dir, tag="checkpoint-best.pth", client_state=client_state)


def load_model(args, model_without_ddp, optimizer, loss_scaler):
    if args.resume:
        if args.resume.startswith('https') or os.path.isfile(args.resume):
            ...
            if hasattr(args, "record_feature") and args.record_feature:
                strict = False
            else:
                strict = True

            # # TODO: debug code, to delete this
            # if "dino_loss.center" in checkpoint['model']:
            #     checkpoint['model']['dino_loss.center'] = checkpoint['model']['dino_loss.center'].mean(dim=1)
    ...
    if hasattr(args, "pretrained") and args.pretrained:
        ...
        print("Load pretrained model msg is {}".format(msg))
        # model_without_ddp.load_state_dict(checkpoint['model'])
        print("Load pretrained checkpoint %s" % args.pretrained)


def load_model_best(args, model):
